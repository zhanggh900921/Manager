...
public class ContentTopology{
	...
	public ContentTopology(DataInputStream dis,ManagmentServer ms) {    
		this.dis = dis;
		this.ms = ms;
        
       
    	net = new TopoNetwork("面向内容网络"); //定义网络
    	net.setXY(160, 50);
	...
	public void drawSubTopology() {
		...
		for(int i =0;i<subNum;i++) {
			subNet[i] = new TopoNetwork("内容逻辑网"+subTopology[i][0]); //定义网络
	    	subNet[i].setXY(50+i*100, 140);
	...
	public void drawSubObject(int[] r,int num,TopoNetwork subNet) {
		...
		for(int i=1; i<r.length;i++) {
			if(r[i]!=0) {
				TopoNode node= new TopoNode("c"+num+"Router"+r[i]);
			...
			for(int j=1; j<r.length;j++) {
				if(r[i]!=0&&r[j]!=0) {
					if(r[i]<r[j] && topologyArray[r[i]-1][r[j]-1]!=0) {
						TopoLink link = new TopoLink("c"+num+"Line"+r[i]+r[j], subNodes.get(r[i]), subNodes.get(r[j])); //通过起点终点，创建链接
	...
	public void drawRouter(double[][] r ) {          //根据矩阵画出网络中的路由器
		...
		for(int i=1;i<r.length+1;i++) {			
			TopoNode node= new TopoNode("contentRouter"+i);
			nodes.put(i, node);		
	...
	public void drawLink(double[][] r ) {                            //画出拓扑中的链路
		for(int i=1;i<nodes.size()+1;i++)
			for(int j=1;j<nodes.size()+1;j++) {
				if(i<j && r[i-1][j-1]!=0) {
					TopoLink link = new TopoLink("cline"+i+j, nodes.get(i), nodes.get(j)); //通过起点终点，创建链接
					link.setShowArrow(false);	
					try{
